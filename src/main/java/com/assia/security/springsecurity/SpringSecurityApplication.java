    ...
    CommandLineRunner commandLineRunner(PatientRepository patientRepository){
        return args -> {
            patientRepository.save(
                    new Patient(null,"assia",new Date(),false,200)
            );
            patientRepository.save(
                    new Patient(null,"imane",new Date(),true,250)
            );
            patientRepository.save(
                    new Patient(null,"Hicham",new Date(),false,290)
            );
            patientRepository.save(
                    new Patient(null,"Asmaa",new Date(),true,190)
            );
            patientRepository.save(
                    new Patient(null,"Anas",new Date(),false,310)
            );
